...
zE('webWidget', 'chat:addTags', ['triveinvest-en']);
...
window.zESettings = {
    webWidget: {
        ...
        chat: {
            ...
            departments: {
                enabled: [],
                select: 'Trive Invest Support Team-EN',
            },
            concierge: {
                name: "Live Chat",
...
function userLoginControl() {
    if (userLog == true) {
        ...
        finalPhone = selectedOptionValue + phoneZendesk;
        finalName = nameZendesk;
...
zE('webWidget:on', 'chat:departmentStatus', function (dept) {
    if (dept.name === "Trive Invest Support Team-EN" && dept.status === 'online') {
        ZendeskButton.addEventListener("click", function () {
            formActive();
        });
    }
    else if (dept.name === "Trive Invest Support Team-EN" && dept.status === 'offline') {
...
function openZendeskWidget(e) {
    ...
    if (valid == false) {
        ...
        setTimeout(function () {
            ...
            msg = "Hi, I'm " + nameZendesk + " I have read and accepted all permissions.     " + message;
            zE('webWidget', 'chat:addTags', ['izinler-onaylandi-en', ticketCategoryValue]);
        }, 500);
        setTimeout(function () {
